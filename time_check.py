...
def check_recording_time(timess):
    try:
        ...
        max_time = datetime.strptime("00:50:00", "%H:%M:%S")  # 300-minute limit

        # Check if the requested time exceeds the 300-minute limit
        if requested_time > max_time:
            return False, "Recording time exceeds the 300-minute limit."
